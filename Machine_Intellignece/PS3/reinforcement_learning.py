...
class RLAgent(Agent[S, A]):
    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:        
        ...
        # Return random action if it should be expolred
...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        # using SARSA equation provided calculate the Q value of the passed state and passed action
        self.Q[state][action] += self.learning_rate * (reward + self.discount_factor * self.compute_q(env, next_state, next_action) - self.Q[state][action])  
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        # To get the utility, get the max q value amon actions avalilble by this state
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        # Calculate q-learing update rule provided for the Q value for each state and action
        next_q_value = 0 if done else self.compute_utility(env, next_state)
        self.Q[state][action] += self.learning_rate * (reward + self.discount_factor * next_q_value - self.Q[state][action])  
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        # get the summation of weights * feature value based on the all features
        return sum([self.weights[action][feature] * val for feature, val in features.items()])
    ...
    # Update the value of Q(state, action) using this transition via the Q-Learning update rule
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        next_q_value = 0 if done else self.__compute_utility_from_features(self.feature_extractor.extract_features(env, next_state))
        
        # Calculate weight table based on features and q_value obtained 
        features = self.feature_extractor.extract_features(env, state)
